...
public class GifAnimator : MonoBehaviour
{
  ...
  public bool isThisMenuGif;
  public bool isStopTheGif;
  public int  stopTheGifIndex;
  ...
  IEnumerator PlayGif(int startIndex)
  {
    ...
    else
    {
      while (isWorking)
      {
        if (index >= stopTheGifIndex && !workOneMoreTime)
        {
          // burda ilk başladıgında durdugu yer
          workOneMoreTime = true;
          isWorking = false;
        ...
        if (index >= FotografSayısı - 1 && workOneMoreTime)
        {
          // burda geri çıktıgı
          isWorking = false;
          image.gameObject.SetActive(false);
  ...
  [ContextMenu("Kapanış")]
  private void Kapanış(int startIndex)
  {
    isWorking = true;
    StartCoroutine(PlayGif(startIndex));
  }
}